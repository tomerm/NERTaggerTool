...
var tsv = (function() {
	...
	return {
					...
					for (j=0; j<ents.length; j++) {
						...
						if (props.type === "meta")
							...
							if (props.offset + props.length <= pureText.length)
								line.entities.push(new Entity(i, props.offset, props.offset + props.length - 1, this.mapEntity(props.type)));
						}
					}
					doc.lines.push(line);
		...
		loadTokenizedData: function(docname, desc, text) {
            ...
            try {
            	for (i=0; i<arr.length;i++) {            		
            		...
            		var token = rowArr[0];
            		if (tdfType == "BU" && rowArr[1].indexOf("_") < 0) {
    					alert("Wrong file format. Check your setting.");
    					return null;            			
            		}
            		var tagArr = tdfType == "BU"? rowArr[1].split("_") : rowArr[1].split("-");
            		var entType = tdfType == "BU"? tagArr[1] : tagArr[0];            		
            		var entName = "";
            		if (tdfType == "BU")
            			entName = tagArr[0];
            		else if(entType != "O")
            			entName = tagArr[1];
            		if (tdfType == "IOB") {
            			for (k=0; k<eTypes.length; k++) {
            				if (entName == eTypes[k].toUpperCase().substring(0,3)) {
            					entName = eTypes[k];
            					break;
            				}
            			}
        			...
        			if (token.length == 1 && !/\d/.test(c) && c < 'A' && !(c=='"' || c=="'" || c=="(" || c==")" || c=="-"))
        				needSpace = false;
        			...
        			if (!eTypes.some(function(name) {
        					if (entName != "" && name.indexOf(entName) === 0) {
        						return true;
        					}
        				})) {
        				if (curEntry != null) {
        			...
        			if (curEntry == null && entType == "continue") {
        				entWithoutBegin++;
        				entType = "begin";
        			}
        			if (entType == "unique" || entType == "begin") {
        				...
        				curEntry.classes = entName.toLowerCase();
        				curEntry.txt = token;
        				...
        				if (curEntry == null) {
        					...
        					alert("Error (line " + countLines + "): entity does't have 'begin' tag. File isn't loaded.");
        					return null;
        				}
        				if(prevSpace) {
        					curEntry.end++;
            ...
            if (entWithoutBegin)
            	alert("Warning: file contains " + entWithoutBegin + " entities without 'begin' tag.");
            window.statusNode.innerHTML = "";
			return doc;
		},
		...
		exportToks: function(fs, exec, doc, path) {
			// Pure tokenization. Each token on its line, empty line between sentences.
			var paras = "";
			for (var i=0; i<doc.lines.length; i++) {
			...
			var result = spawn("java -Dfile.encoding=UTF-8 -jar " + jarDir + "\\ws.jar", obj);
			var err = false;
			if (!result.stderr) {
				alert(result.stderr);
				err = true;
			}
			else if(result.stdout.indexOf("Error") >= 0) {
				alert (result.stdout);
				err = true;
			}
			if (err)
		...
		exportEng: function(fs, exec, doc, path) {
			var paras = "";
			for (var i=0; i<doc.lines.length; i++) {
				paras += doc.lines[i].text + "\n"; 
			}
			paras += "+END+";
			obj = {"input": paras, "shell": true};
			var result = spawn("java -Dfile.encoding=UTF-8 -jar " + jarDir + "\\we.jar " + path, obj);
			if (!result.stderr) {
				alert(result.stderr);
			}
		...
		handleEngineResponce : function(responce) {
			...
			for (var i=0; i<doc.lines.length; i++) { 
				for (var j=0; j<doc.lines[i].entities.length; j++) {
					var ent = doc.lines[i].entities[j];
					ent.line = i;
					ent.txt = doc.lines[i].text.substring(ent.begin, ent.end);
					ent.found = false;
					ent.foundType = 0;
					comp.gold.push(ent);
				}
		...
		cleanSpecial : function(txt) {
			return txt.replace(/\u05be/g,"-").replace(/\u2013/g,"-").replace(/\u2014/g,"-").replace(/\u201c/g,"\"").replace(/\u201d/g,"\"").replace(/\u2018/g,"'").replace(/\u2019/g,"'");
		},
		
		handleExternalResults : function(txt) {
			var tmap = {"PER": "person", "LOC": "location", "ORG": "organization"};
			text = txt.replace(/\r/g, '').replace(/\t/g, ' ');
			var arr = text.split("\n");	
			var comp = new Comp();
			for (var i=0; i<doc.lines.length; i++) { 
				for (var j=0; j<doc.lines[i].entities.length; j++) {
			...
			var startPos = 0;
			var prevEmpty = false;
			var sentence = this.cleanSpecial(doc.lines[0].text);
			var pSentence = this.cleanSpecial(doc.lines[0].text.replace(/\s/g,""));
			var posInPs = 0;
			var iPred = 0;
			var actEnt = null;
			var actType = null;
			for (var i=0; i < arr.length; i++) {
				if (arr[i].trim() == "") {
					...
					prevEmpty = true;
					iSentence++;
					if (iSentence == doc.lines.length)
						break;
					pSentence = this.cleanSpecial(doc.lines[iSentence].text.replace(/\s/g,""));					
					posInPs = 0;
					sentence = this.cleanSpecial(doc.lines[iSentence].text);
					startPos = 0;
					continue;
				}
				prevEmpty = false;
				var isWrong = false;
				var tokAndTag = arr[i].split(" ");
				tokAndTag[0] = this.cleanSpecial(tokAndTag[0]);				
				if (tokAndTag.length != 2)
					isWrong = true;
				else {
					var tc = tokAndTag[1].split("");
					if ((tc[0] != 'B' && tc[0] != 'I' && tc[0] != 'O') || (tc[0] != "O" && tc[1] != "-"))
						isWrong = true;
				}
				if (isWrong) {
					alert ("Choosen file has a wrong format.\nLine " + i + ", text: " + arr[i]);
					return;
				}
				
				if (pSentence.indexOf(tokAndTag[0], posInPs) != posInPs) {
					alert("Text contents of current document and choosen file are different.\nLine " + i + ", text: " + arr[i] + "\nDoc line: " + iSentence + "\nText: \n" + doc.lines[iSentence].text);
					/*
					alert("Pure text: " + pSentence + "\nText: " + tokAndTag[0] + "\nInd: " + posInPs + "\nAct.ind: " + pSentence.indexOf(tokAndTag[0], posInPs));
					for (var k=0; k<tokAndTag[0].length; k++) {
						alert("" + k + ". Char in ind: " + pSentence.charCodeAt(posInPs+k) + "\nChar in text: " + tokAndTag[0].charCodeAt(k));
					}
					*/
					return;
				}
				posInPs += tokAndTag[0].length;
				var type = tokAndTag[1].indexOf("-") > 0? tokAndTag[1].split("-")[1] : "O";
				var tag = tokAndTag[1].substring(0,1);
				var offs = sentence.indexOf(tokAndTag[0], startPos);
				//if (iSentence == 1)
				//	alert("Got token: " + tokAndTag[0] + ", tag: " + tokAndTag[1] + ", startPos: " + startPos + ", offs: " + offs);

				if (tmap.hasOwnProperty(type)) {
					if (!actEnt) {
						actEnt = new Entity(iSentence, offs, offs + tokAndTag[0].length - 1, tmap[type], tokAndTag[0]);
						actType = type;
					}
					else if(actType == type && tag != "B") {
						actEnt.end = offs + tokAndTag[0].length - 1;
						actEnt.txt = sentence.substring(actEnt.begin, actEnt.end+1);
					}
					else {
						comp.actual.push(actEnt);
						//if (iSentence == 1)
						//	alert("Add entity: line: " + iSentence + ", type: " + actEnt.classes + ", text: " +  sentence.substring(actEnt.begin, actEnt.end+1));
						actEnt = new Entity(iSentence, offs, offs + tokAndTag[0].length - 1, tmap[type], tokAndTag[0]);
						actType = type;
					}
				}
				...
				startPos = offs + tokAndTag[0].length;
			} 
			} catch (e) {alert("" + iSentence + "\n" + e)}
			return comp;
		},
		...
		refreshLine: function(div) {   // I am here
			...
			doc.lines[q].text = "";
			doc.lines[q].entities = [];
			doc.lines[q].meta = "updated";
			div.setAttribute("data-meta", "updated");
			var prevSpace = false;
			for (ip=q; ip<q+1; ip++) {   //TBD
				var entities = [];
				var ready = false, thesame = true;
